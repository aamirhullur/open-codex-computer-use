...
// snapshot, so the modern capture path is exercised without a Linux desktop.
func fakeSnapshotRunner(snapshot *appSnapshot) func(linuxRequest) (*linuxResponse, error) {
	return func(req linuxRequest) (*linuxResponse, error) {
		return &linuxResponse{OK: true, Snapshot: snapshot}, nil
	}
}

func cannedSnapshot() *appSnapshot {
	return &appSnapshot{
		App:                 appDescriptor{Name: "Text Editor", BundleIdentifier: "org.gnome.TextEditor", PID: 4321},
...
func TestModernGetAppStateMintsHandle(t *testing.T) {
	...
	result := svc.callTool("get_app_state", map[string]any{"app": "Text Editor"}, true)
	if result.IsError {
	...
	if app["name"] != "Text Editor" || app["bundle_identifier"] != "org.gnome.TextEditor" || app["pid"] != 4321 {
...
func TestModernGetAppStateSupersedesOnReCapture(t *testing.T) {
	...
	first := svc.callTool("get_app_state", map[string]any{"app": "Text Editor"}, true)
	second := svc.callTool("get_app_state", map[string]any{"app": "Text Editor"}, true)
...
func TestLegacyGetAppStateNoStructuredContent(t *testing.T) {
	...
	result := svc.callTool("get_app_state", map[string]any{"app": "Text Editor"}, false)
	if result.IsError {
