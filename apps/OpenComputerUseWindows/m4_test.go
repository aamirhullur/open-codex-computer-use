...
func (r *actionRunner) run(req psRequest) (*psResponse, error) {
	if req.Tool == "get_app_state" || req.Tool == "list_apps" {
		return &psResponse{OK: true, Snapshot: r.snap}, nil
	}
	r.mu.Lock()
	r.actionCalls++
	r.mu.Unlock()
...
func mintHandle(t *testing.T, svc *service) string {
	t.Helper()
	res := svc.callTool("get_app_state", map[string]any{"app": "Notepad"}, true)
...
func allActionTools() map[string]map[string]any {
	return map[string]map[string]any{
		"click":                    {"app": "Notepad", "element_index": 0},
		"perform_secondary_action": {"app": "Notepad", "element_index": 0, "action": "invoke"},
		"scroll":                   {"app": "Notepad", "element_index": 0, "direction": "down"},
		"drag":                     {"app": "Notepad", "from_x": 1, "from_y": 1, "to_x": 2, "to_y": 2},
		"type_text":                {"app": "Notepad", "text": "hi"},
		"press_key":                {"app": "Notepad", "key": "Return"},
		"set_value":                {"app": "Notepad", "element_index": 0, "value": "x"},
	}
}
...
func TestModernActionMalformedRef(t *testing.T) {
	svc, fr := newActionService()
	res := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": "not-a-handle"}, true)
...
func TestModernActionUnknownRef(t *testing.T) {
	...
	res := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": unminted}, true)
...
func TestModernActionExpiredRef(t *testing.T) {
	...
	res := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": ref}, true)
...
func TestModernActionStaleRef(t *testing.T) {
	...
	res := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": first}, true)
...
func TestModernActionIdentityMismatch(t *testing.T) {
	...
	retry := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": ref}, true)
	if retry.IsError {
...
func TestModernActionRuntimeElementMismatch(t *testing.T) {
	svc, fr := newActionService()
	fr.resp = &psResponse{OK: false, ErrorKind: gomcp.ErrorKindElementMismatch, Error: "resolved element no longer matches"}
	ref := mintHandle(t, svc)
...
func TestModernActionUncertain(t *testing.T) {
	...
	res := svc.callTool("type_text", map[string]any{"app": "Notepad", "text": "hi", "snapshot_ref": ref}, true)
...
func TestModernActionRefreshFailure(t *testing.T) {
	svc, fr := newActionService()
	fr.resp = &psResponse{OK: true, Snapshot: nil}
	ref := mintHandle(t, svc)
...
func TestModernActionPreDispatchFailureRestoresLive(t *testing.T) {
	...
	for _, p := range []struct {
		...
		bad := svc.callTool("click", map[string]any{"app": "Notepad", "x": p.x, "y": p.y, "snapshot_ref": ref}, true)
		if !bad.IsError {
	...
	good := svc.callTool("click", map[string]any{"app": "Notepad", "x": 2399, "y": 1599, "snapshot_ref": ref}, true)
	if good.IsError {
...
func TestModernActionRejectedBeforeInput(t *testing.T) {
	svc, fr := newActionService()
	fr.resp = &psResponse{OK: false, ErrorKind: gomcp.ErrorKindRejectedBeforeInput, Error: "unknown element_index"}
	ref := mintHandle(t, svc)

	res := svc.callTool("set_value", map[string]any{"app": "Notepad", "element_index": 0, "value": "x", "snapshot_ref": ref}, true)
	if !res.IsError {
	...
	retry := svc.callTool("set_value", map[string]any{"app": "Notepad", "element_index": 0, "value": "x", "snapshot_ref": ref}, true)
	if retry.IsError {
...
func TestModernActionWhitespaceRefIsMissing(t *testing.T) {
	svc, fr := newActionService()
	res := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": "   "}, true)
...
func TestModernActionSuccessChain(t *testing.T) {
	...
	res := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": ref1}, true)
	if res.IsError {
	...
	old := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": ref1}, true)
	if errorCode(old) != gomcp.ErrSnapshotRefStale {
		t.Fatalf("old handle after success = %+v, want stale", old)
	}

	next := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": ref2}, true)
	if next.IsError {
...
func TestModernActionConcurrentDoubleUse(t *testing.T) {
	...
	go func() {
		done <- svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": ref}, true)
	}()
	...
	loser := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0, "snapshot_ref": ref}, true)
...
func TestLegacyActionZeroStoreInteraction(t *testing.T) {
	...
	if res := svc.callTool("get_app_state", map[string]any{"app": "Notepad"}, false); res.IsError {
		t.Fatalf("legacy get_app_state errored: %+v", res)
	}
	res := svc.callTool("click", map[string]any{"app": "Notepad", "element_index": 0}, false)
	if res.IsError {
