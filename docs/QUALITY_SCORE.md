...
| 测试 | B | `swift test` + smoke suite 已覆盖 9 个 tools 的回归；dual-era MCP `2026-07-28` 迁移新增了三端共享的协议 golden fixtures（接入 `scripts/ci.sh`）、snapshot handle store 与事务性 modern action 的单测，以及包含真实 handle 链和 stale-reuse 拒绝的 modern smoke。 | 增加更多普通 app 的录制回归，减少只依赖 fixture 和一次性手工检查；补齐官方 conformance suite 支持 `2026-07-28` 后的端到端校验。 |
| 可观测性 | C | 已有 `doctor`、`snapshot`、smoke 输出，以及一组仓库内留档的官方 `computer-use` / 本仓库实现对比样本；modern MCP 路径新增了 stderr-only 结构化调试日志（platform / era / method / result / duration / redacted handle suffix）和 snapshot handle 计数器。 | 把 MCP-scoped 的结构化日志扩展成统一日志级别，补失败上下文和 release artifact 里的诊断信息，把一次性样本收敛成可重复采集流程。 |
